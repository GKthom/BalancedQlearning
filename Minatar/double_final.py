#sample call:
#python double_final.py -g 'game' -v
...
#
################################################################################################################
def world_dynamics(t, replay_start_size, num_actions, s, env, policy_net,policy_net_double):
    ...
    if t < replay_start_size:
        ...
        if numpy.random.binomial(1, epsilon) == 1:
            ...
            with torch.no_grad():
                comb_net=policy_net(s)+policy_net_double(s)
                action = comb_net.max(1)[1].view(1, 1)
...
#
################################################################################################################
def train(sample, policy_net, target_net, policy_net_double, target_net_double, optimizer,optimizer_double):
    ...
    if numpy.random.rand()>0.5:
        ...
        Qt=target_net_double(none_terminal_next_states)
        Qt2=target_net(none_terminal_next_states)
        maxval, maxinds=torch.max(Qt2,dim=1)
        inds=maxinds.long()
        Qtarg=Qt.gather(1,inds.view(-1,1))
        Qmod_targ=torch.zeros(len(sample), 1, device=device)
        if len(none_terminal_next_states) != 0:
            Qmod_targ[none_terminal_next_state_index]=Qtarg
        # Compute the target
        target = rewards + GAMMA * Qmod_targ
        # Huber loss
        ...
        Qt=target_net(none_terminal_next_states)
        Qt2=target_net_double(none_terminal_next_states)
        Q_s_a = policy_net_double(states).gather(1, actions)
        maxval, maxinds=torch.max(Qt2,dim=1)
        inds=maxinds.long()
        Qtarg=Qt.gather(1,inds.view(-1,1))
        Qmod_targ=torch.zeros(len(sample), 1, device=device)
...
# double dqn
#
# Double DQN algorithm with the option to disable replay and/or target network, and the function saves the training data.
#
# Inputs:
#   env: environment of the game
...
#
#################################################################################################################
def doubledqn(run_no, env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE):
    print(run_no)
    ...
    policy_net_double = QNetwork(in_channels, num_actions).to(device)
    replay_start_size = 0
    if not target_off:
        target_net = QNetwork(in_channels, num_actions).to(device)
        target_net.load_state_dict(policy_net.state_dict())
        target_net_double = QNetwork(in_channels, num_actions).to(device)
        target_net_double.load_state_dict(policy_net.state_dict())
    ...
    optimizer_double = optim.RMSprop(policy_net_double.parameters(), lr=step_size, alpha=SQUARED_GRAD_MOMENTUM, centered=True, eps=MIN_SQUARED_GRAD)
    # Set initial values
    e_init = 0
    t_init = 0
    policy_net_update_counter_init = 0
    policy_net_double_update_counter_init = 0
    avg_return_init = 0.0
    data_return_init = []
    frame_stamp_init = []
    ...
    if load_path is not None and isinstance(load_path, str):
        ...
        optimizer_double.load_state_dict(checkpoint['optimizer_state_dict'])

        e_init = checkpoint['episode']
        t_init = checkpoint['frame']
        policy_net_update_counter_init = checkpoint['policy_net_update_counter']
        policy_net_double_update_counter_init = checkpoint['policy_net_double_update_counter']
        ...
        policy_net.train()
        policy_net_double.train()
        if not target_off:
            target_net.train()
            target_net_double.train()
    ...
    policy_net_double_update_counter = policy_net_double_update_counter_init
    t_start = time.time()
    reward_seq=[]
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            # Generate data
            #blah=1
            s_prime, action, reward, is_terminated = world_dynamics(t, replay_start_size, num_actions, s, env, policy_net, policy_net_double)
            sample = None
            if replay_off:
            ...
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    train(sample, policy_net, policy_net, policy_net,policy_net, optimizer,optimizer_double)
                else:
                    policy_net_update_counter += 1
                    train(sample, policy_net, target_net,policy_net_double,target_net_double,optimizer, optimizer_double)
        ...
        if store_intermediate_result and e % 1000 == 0:
            torch.save({
                        ...
                        'optimizer_double_state_dict': optimizer_double.state_dict(),
    ...
    #log_filename="log_DQN_"+".npy.npz"
    #numpy.savez(log_filename,reward_seq)
    # Write data to file
    torch.save({
        'reward_seq': reward_seq,
        'returns': data_return,
        ...
        'policy_net_state_dict': policy_net.state_dict()
    }, output_file_name + str(run_no)+"_data_and_weights_double")
...
def main(run_no):
    ...
    doubledqn(run_no, env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha)
