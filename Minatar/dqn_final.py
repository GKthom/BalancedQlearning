...
def train(sample, policy_net, target_net, optimizer):
    ...
    #print(target_net(none_terminal_next_states).detach())
    ...
    #print(rewards-Q_s_a+ GAMMA*Q_s_prime_a_prime)
    #time.sleep(4)
    # Compute the target
...
#
#################################################################################################################
def dqn(run_no, env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE):
    print(run_no)
    ...
    torch.save({
        ...
        'policy_net_state_dict': policy_net.state_dict()
    }, output_file_name + str(run_no)+"_data_and_weights_dqn")
...
def main(run_no):
    ...
    dqn(run_no, env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha)
