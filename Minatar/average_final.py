#sample call:
#python average_final.py -g 'game' -v
...
#
################################################################################################################
def train(sample, policy_net, target_net, optimizer,target_net_all,NQ,num_actions):
    ...
    #avgQ=torch.zeros(len(sample),num_actions,device=device)
    #print(target_net_all[0](states))
    ...
    avgQ=torch.zeros(len(none_terminal_next_states),num_actions,device=device)
    #Find average Q target
    for i in range(NQ):
        avgQ=avgQ+target_net_all[i](none_terminal_next_states)
    avgQ=avgQ/NQ

    Q_s_prime_a_prime = torch.zeros(len(sample), 1, device=device)
    if len(none_terminal_next_states) != 0:
        Q_s_prime_a_prime[none_terminal_next_state_index] = avgQ.detach().max(1)[0].unsqueeze(1)
    #Compute target
...
#
#################################################################################################################
def averagedqn(run_no, env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE):
    ...
    #Create NQ target networks
    target_net_all = [QNetwork(in_channels, num_actions).to(device) for k in range(NQ)]
    replay_start_size = 0
    if not target_off:
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    train(sample, policy_net, policy_net, optimizer,target_net_all,NQ,num_actions)
                else:
                    policy_net_update_counter += 1
                    train(sample, policy_net, target_net, optimizer,target_net_all,NQ,num_actions)
            ...
            if not target_off and policy_net_update_counter > 0 and policy_net_update_counter % TARGET_NETWORK_UPDATE_FREQ == 0:
                #Replace the appropriate target network
                target_net_all[targnet_counter%NQ].load_state_dict(policy_net.state_dict())
                targnet_counter+=1
            G += reward.item()
    ...
    torch.save({
        ...
        'policy_net_state_dict': policy_net.state_dict()
    }, output_file_name + str(run_no)+"_data_and_weights_average")
...
def main(run_no):
    ...
    averagedqn(run_no, env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha)
