#sample call:
#python maxmin_final.py -g 'game' -v
...
#
################################################################################################################
def world_dynamics(t, replay_start_size, num_actions, s, env, policy_net,policy_solvers,NQ):
    ...
    if t < replay_start_size:
        ...
        if numpy.random.binomial(1, epsilon) == 1:
            ...
            minQ_pols=Qmin(policy_solvers,NQ,num_actions,s)
            with torch.no_grad():
...
##########################################################################################################
def Qmin(policy_solvers,NQ,num_actions,s):
    finQ=torch.zeros(1,num_actions,device=device)
    for p in range(num_actions):
        selQ=torch.zeros(1,NQ,device=device)
        for q in range(NQ):
            selQ[0][q]=policy_solvers[q](s)[0][p]
        finQ[0][p]=torch.min(selQ)
    return finQ
#######################################################################################################
#Find minimum of all Q networks
##########################################################################################################
def Qminbatch(target_solvers,NQ,num_actions,states):
    stackedtens=torch.zeros(len(states),num_actions,NQ,device=device)
    for i in range(NQ):
...
#
################################################################################################################
def train(sample, policy_solvers, target_solvers, agent_id, optimizer,num_actions,NQ,optimizers):
    ...
    Q_s_a = policy_solvers[agent_id](states).gather(1, actions)
    ...
    mintargpols=Qminbatch(target_solvers,NQ,num_actions,next_states)
    Q_s_prime_a_prime = torch.zeros(len(sample), 1, device=device)
    if len(none_terminal_next_states) != 0:
        Q_s_prime_a_prime[none_terminal_next_state_index] = mintargpols[none_terminal_next_state_index].detach().max(1)[0].unsqueeze(1)
...
#
#################################################################################################################
def maxmindqn(run_no, env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE):
    print(run_no)
    # Get channels and number of actions specific to each game
    in_channels = env.state_shape()[2]
    num_actions = env.num_actions()
    # Instantiate networks, optimizer, loss and buffer
    policy_solvers = [QNetwork(in_channels, num_actions).to(device) for k in range(NQ)]
    target_solvers = [QNetwork(in_channels, num_actions).to(device) for k in range(NQ)]
    ...
    optimizers=[]
    for i in range(NQ):
        optimizers.append(optim.RMSprop(policy_solvers[i].parameters(), lr=step_size, alpha=SQUARED_GRAD_MOMENTUM, centered=True, eps=MIN_SQUARED_GRAD))
    ...
    if load_path is not None and isinstance(load_path, str):
        ...
        if not target_off:
            target_net.train()
        for i in range(NQ):
            policy_solvers[i].train()
            target_solvers[i].train()
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            if t%10000==0:
                print(t)
            # Generate data
            s_prime, action, reward, is_terminated = world_dynamics(t, replay_start_size, num_actions, s, env, policy_net,policy_solvers,NQ)
            sample = None
            if replay_off:
            ...
            agent_id=numpy.random.randint(NQ)
            # Train every n number of frames defined by TRAINING_FREQ
            if t % TRAINING_FREQ == 0 and sample is not None:
                if target_off:
                    train(sample, policy_solvers, policy_solvers,agent_id, optimizer,num_actions,NQ,optimizers)
                else:
                    policy_net_update_counter += 1
                    train(sample, policy_solvers, target_solvers,agent_id, optimizer,num_actions,NQ,optimizers)
            ...
            if not target_off and policy_net_update_counter > 0 and policy_net_update_counter % TARGET_NETWORK_UPDATE_FREQ == 0:
                target_solvers[agent_id].load_state_dict(policy_solvers[agent_id].state_dict())
    ...
    torch.save({
        ...
        'policy_net_state_dict': policy_net.state_dict()
    }, output_file_name + str(run_no)+"_data_and_weights_maxmin"+str(NQ))
...
def main(run_no):
    ...
    maxmindqn(run_no, env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha)
